...
- 🚧 **Plan G** 0/21 (`2026-04-28-plan-g-georeferenced-ifc.md`) — Coordinate System & Georeferenced IFC. Plan kirjoitettu, MODE A toteutus alkaa. 6 sectionia: CRSConfig + storey_z_levels (S1, 4t) → IfcProjectedCRS + IfcMapConversion (S2, 3t) → Site/Building/Storey-hierarkia (S3, 3t) → element-add storey-aware (S4, 3t) → CLI/GUI/validate (S5, 3t) → integraatio/docs/loppupiste (S6, 5t). Avain-päätökset:
  - Default CRS: ETRS-TM35FIN, IfcProjectedCRS-kentät: `Name="EPSG:3067"`, `Description="ETRS-TM35FIN"`, `GeodeticDatum="ETRS89"`
  ...
  - `storey_z_levels_mm` profiilissa (lista mm-arvoja, esim. `[0, 3500, 7000]`); default `[0.0]` (1 kerros)
