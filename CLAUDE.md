...
- ✅ Plan C **12/12 valmis** (`2026-04-27-plan-c-ifcsystem-grouping.md`) — IfcSystem-ryhmittely (Refrigeration LT/MT, Drainage, Cable carriers, Refrigeration plant) IfcRelAssignsToGroup-relaatiolla. 151 testiä passed, coverage 91 %. Loppupisteen SHA: `7e09716`.
- ✅ Plan D **25/25 valmis** (`2026-04-27-plan-d-pyside6-gui.md`) — PySide6-desktop-GUI wrappaa CLI-coren: MainWindow + brand-paletti (slate gradient + amber primary), DXF-picker + taustasäikeen konvertointi, layer-preview Talo2000-resoluutiolla, profiili-editori (custom layer-säännöt UI:ssa, TOML-roundtrip), about-dialog ja QSettings-pohjainen recent-files. 200 testiä passed, coverage 89 %, ruff clean. Loppupisteen SHA: `011bd5e`. Plans E–F kirjoittamatta.
...
- **Plan E:** ✅ kirjoitettu `docs/superpowers/plans/2026-04-27-plan-e-pyinstaller.md` (23 tehtävää, 5 sectionia). PyInstaller-pakkaus + GitHub Releases: bootstrap, .spec datas + hidden imports + version_info, Windows build + CI matrix, release-workflow + checksum + CHANGELOG, smoke + dokumentointi. Toteutus alkaa Mode A:ssa Task 1:llä — ks. `PROGRESS.md`.
