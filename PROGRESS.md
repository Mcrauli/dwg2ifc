...
- [ ] **Bugfix 11** — **GUI Profile editor RAVA-laajennus** (Plan H jätti tämän tekemättä — section reorganisoitu): Profile editor:in Rule-dialogissa ei ole UI-kenttää RAVA-koodeille (lvi_code / talotekniikka_code) eikä domain-valitsinta (ARK/TATE). Käyttäjä näkee säännön vain Talo2000-pohjaisesti, vaikka TOML-data sisältää uudet kentät. Korjaus: a) lisää `RuleEditDialog`:iin domain-pudotusvalikko (ARK/TATE), b) ehdollinen kenttä-näyttö: jos domain=ARK näytä `talo2000_code`, jos TATE näytä `lvi_code` JA `talotekniikka_code` (vain yksi täytetään, dropdown ehdottaa virallisia koodeja `LVI_TUOTEOSA`/`TALOTEKNIIKKA_TUOTEOSA` -konstanteista jotka Plan H Section 2 latasi), c) Layer-table-näkymä päivittyy näyttämään käytetty koodisto (esim. uusi sarake "Domain" tai "Codeset"), d) Pydantic-validator UI-puolella: estää tallennuksen jos ARK+RAVA-koodi tai TATE+talo2000-koodi yhdistelmä. Tiedostot: `src/dxf2ifc/gui/rule_dialog.py`, `src/dxf2ifc/gui/profile_editor.py`, `src/dxf2ifc/gui/layer_table.py`, vastaavat testit.

Bugfix kierros 3 ajoitus: kun Plan H valmistuu, käy nämä läpi: Bugfix 7 (geometria) → Bugfix 8 (TIKAS) → Bugfix 9 (Profile Load) → **Bugfix 11 (GUI RAVA-laajennus)**. Sit Plan G.
