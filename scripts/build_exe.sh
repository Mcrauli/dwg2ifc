#!/usr/bin/env bash
# Local Linux PyInstaller smoke build for dwg2ifc.
# Validates that build/dwg2ifc.spec is well-formed; the resulting ELF is
# only useful for local sanity checking — Windows .exe ships from CI.
# Usage: DXF2IFC_VERSION=0.1.0 ./scripts/build_exe.sh

set -euo pipefail

VERSION="${DXF2IFC_VERSION:-$(grep -oE '__version__\s*=\s*"[^"]+"' src/dwg2ifc/_version.py | head -n1 | sed -E 's/.*"([^"]+)".*/\1/')}"

echo "Building dwg2ifc smoke binary version ${VERSION}..."

uv sync --extra dev --extra gui
uv run pyinstaller build/dwg2ifc.spec --clean --noconfirm

SOURCE="dist/dwg2ifc"
TARGET="dist/dwg2ifc-${VERSION}"
