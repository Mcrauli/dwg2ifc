...
and the existing :func:`dwg2ifc.core.ifc_writer.convert_dxf` path reads that
DXF.

Why accoreconsole + DXFOUT and not ODA File Converter:

* dwg2ifc already invokes ``accoreconsole.exe`` for STLOUT ACIS tessellation
  (:mod:`dwg2ifc.core.preprocessing`), so there are zero additional binary
...
machine, then ``--magicad-ifc`` merge in dwg2ifc.
"""
