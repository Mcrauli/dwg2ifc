    ...
    for entity in entities:
        ...
        result.append(
            MappedEntity(
                ...
                # ``block_attribs`` carries the INSERT's ATTRIB fields.
                # ``orchestrator._process_one_file`` calls
                # ``apply_block_attribs`` later which routes them into
                # ``fi_tuote`` / ``fi_tekninen``; without propagating
                # them here that merge sees an empty list and the
                # user's per-device tech-spec values never reach Solibri.
                block_attribs=list(entity.block_attribs),
