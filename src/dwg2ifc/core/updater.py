...
Polls GitHub Releases for newer versions of dwg2ifc and exposes a
...
so the update flow is:
    ...
    2. Rename current ``dwg2ifc-X.Y.Z.exe`` → ``…X.Y.Z.exe.old``
...
When running from source (``python -m dwg2ifc.gui``) the entire
...
DEFAULT_REPO: str = "Mcrauli/dwg2ifc"
GITHUB_API_URL: str = "https://api.github.com/repos/{repo}/releases"
USER_AGENT: str = f"dwg2ifc-updater/{__version__}"
...
def is_running_bundled() -> bool:
    ...
    Self-update is meaningless when the user runs ``python -m dwg2ifc.gui``
    against a source checkout — the swap target would be the Python
    interpreter, not the dwg2ifc exe.
    """
...
def _restart_log_path() -> Path:
    ...
    """
    return Path(tempfile.gettempdir()) / "dwg2ifc_restart.log"
...
def _spawn_delayed_launcher(
    ...
    *,
    ...
    A breadcrumb log is appended to ``%TEMP%\\dwg2ifc_restart.log`` on
    ...
    batch_dir = Path(tempfile.mkdtemp(prefix="dwg2ifc_restart_"))
