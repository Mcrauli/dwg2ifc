...
# `sys.executable` is dwg2ifc.exe, so this re-launches the GUI as soon as
...
from dwg2ifc.core.updater import cleanup_old_exe
...
def _set_app_user_model_id() -> None:
    """Tell Windows that this process is the dwg2ifc app (not python.exe).
...
def _load_app_icon() -> QtGui.QIcon | None:
    """Return the dwg2ifc application icon as a QIcon.

    Loads from the bundled package resource so the same call works in
    both the frozen exe (``dwg2ifc.gui/dwg2ifc.ico`` inside _MEIPASS)
    and a source checkout (``assets/dwg2ifc.ico`` via importlib).
    Falls back to the PNG when the .ico is unavailable. Returns
    ``None`` and lets Qt use the default if neither asset is present.
    """
    for filename in ("dwg2ifc.ico", "dwg2ifc.png"):
        try:
            ref = resources.files("dwg2ifc.gui").joinpath(filename)
            if ref.is_file():
...
def run(argv: list[str] | None = None) -> int:
    ...
    # ``dwg2ifc.exe convert <dxf> <ifc> ...`` (or any other CLI
    # subcommand) we bypass the Qt event loop and route to
    # :func:`dwg2ifc.cli.main`. Without this hop the GUI window opens
    ...
    if len(args) > 1 and (
        ...
        from dwg2ifc.cli import main as cli_main
